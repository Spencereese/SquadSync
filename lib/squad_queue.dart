...
import 'utils.dart';
import 'performance_hub_tab.dart'; // Import the new tab
                ...
                ElevatedButton(
                  onPressed: () {
                    setState(() {
                      ...
                      gameHistory.last['ratings'] = ratings.map(
                        (key, value) => MapEntry(
                      ...
                      for (var player in walkingPlayers) {
                        for (var category in [
                          ...
                          if (ratings[player]![category] != null) {
                            dailyRatings[player]![category]!
                                .add(ratings[player]![category]!.toInt());
                            allTimeRatings[player]![category]!
                                .add(ratings[player]![category]!.toInt());
                            if (dailyRatings[player]![category]!.isNotEmpty &&
                                now.day != today.day) {
                              dailyRatings[player]![category] = [
                                ratings[player]![category]!.toInt()
                              ];
                            }
                          }
                        }
