...
class OnboardingService extends _$OnboardingService {
  ...
  bool isStepValid(int step) {
    if (step == 0) {
      ...
      return (state.value?.displayName?.length ?? 0) > 3 && state.value?.avatarUrl != null;
    } else if (step == 1) {
  ...
  Future<void> updateProfile(String displayName, String avatarUrl) async {
    state = AsyncValue.data(state.value!.copyWith(displayName: displayName, avatarUrl: avatarUrl));
    await saveDraft();
  }
  ...
  Future<void> nextStep() async {
    if (isStepValid(state.value!.currentStep)) {
      state = AsyncValue.data(state.value!.copyWith(currentStep: state.value!.currentStep + 1));
      await saveDraft();
    }
  }
    ...
    if (draft != null) {
      ...
      return OnboardingState(
        ...
        pinnedGames: data['pinnedGames'] != null
            ? List<Map<String, dynamic>>.from(json.decode(data['pinnedGames']))
            : [],
        isInitialized: true,
      );
    }
