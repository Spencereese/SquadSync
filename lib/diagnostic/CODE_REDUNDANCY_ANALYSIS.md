...
**Scope**: Complete lib/ folder (~233 .dart files, ~147,200 LOC)

---

## Current Architecture Status

**Architecture Pattern**: Clean Architecture + Riverpod (streamlined)  
...
## 🎯 Current Issues & Suggested Fixes

### Critical Refactoring Targets

#### 1. **God File: chat_info_screen.dart (3,155 lines)**
**Problem**: Single file handling chat UI, members, settings, media gallery, polls  
**Suggested Split**:
- `chat_info_screen.dart` (300 lines) - Main coordinator
- `chat_members_section.dart` (400 lines) - Member management
- `chat_media_gallery.dart` (600 lines) - Media browsing
- `chat_settings_section.dart` (400 lines) - Settings UI
- `chat_polls_section.dart` (300 lines) - Poll management
- `chat_reactions_section.dart` (200 lines) - Reactions UI
- `chat_info_header.dart` (150 lines) - Header component
- `chat_info_actions.dart` (250 lines) - Action buttons

**Impact**: Improved maintainability, easier testing, clearer responsibilities

---

#### 2. **Large Notifiers Need Splitting**

**ChatNotifier (927 lines)**:
- Too many responsibilities: messages, reactions, typing, polls, media
- **Suggest**: Split into MessageNotifier + PollNotifier + MediaNotifier

**SquadNotifier (509 lines)**:
- Handles spots, timers, peacock queue, member status
- **Suggest**: Extract TimerManagementNotifier + SpotManagementNotifier

---

#### 3. **Duplicate Game Selection Components**
**Files with similar functionality**:
- `lib/chat/game_selection_sheet.dart`
- `lib/lobbies_tab/game_search_widget.dart`
- Multiple game selector dialogs

**Suggest**: Create single `lib/shared/widgets/game_selector/` module with:
- `game_selector_widget.dart` - Reusable component
- `game_search_delegate.dart` - Search functionality
- `game_tile.dart` - Display component

---

#### 4. **Print Statements in Production Code**
**Files with print() calls** (should use logging framework):
- `lib/profile_tab.dart` (2 occurrences)
- `lib/lobbies_tab/lobbies_tab.dart` (4 occurrences)
- `lib/chat/lobbies_screen.dart` (1 occurrence)
- `lib/chat/spots_sheet.dart` (1 occurrence)
- `lib/screens/discovery_screen.dart` (1 occurrence)
- `lib/main.dart` (2 occurrences)

**Suggest**: Replace with proper logging service (logger package)

---

#### 5. **Type Compatibility Issues**
**lib/screens/squad_detail_screen.dart**:
- `Lobby` vs `PublicSquad` type mismatch (2 errors)
- **Suggest**: Standardize on single entity type or create adapter

**lib/widgets/spots_lobby_bar.dart**:
- `String` vs `int` parameter mismatch in claimSpot/unclaimSpot (2 errors)
- **Suggest**: Update method signatures to accept int consistently
...
**Current State**: 28 service files with varying sizes (200-1,200 lines)  
**Large Services**:
- `VideoService` (1,212 lines) - Agora integration
- `VoiceService` (887 lines) - Voice SDK
- `ChatUIManager` (767 lines) - UI coordination
- `MessageService` (677 lines) - Messaging logic

**Suggest**: Consider breaking large services into focused modules

---

#### 2. **Direct Database Access Removal**
Some UI components still access Supabase directly instead of through repositories.

**Files to Refactor**:
- `lib/presentation/notifiers/current_lobby_notifier.dart` - Uses SupabaseClient directly
- `lib/presentation/notifiers/user_squads_notifier.dart` - Direct Realtime subscriptions
- `lib/presentation/notifiers/discovery_notifier.dart` - Bypasses repository layer

**Suggest**: All database access should go through repository implementations

---

#### 3. **Integration Tests Need Updates**
**Current State**: 90 errors in integration_test/ files  
**Cause**: Tests reference deleted Use Case layer

**Suggest**: Update test mocks to use repository pattern:
```dart
// OLD: Mock use cases
final mockFetchGames = MockFetchGames();
...
#### 1. **Caching Strategy**
**Suggest**: Implement smarter cache invalidation in SQLite layer
- Current: Cache everything with 100ms validity
- Better: LRU cache with configurable TTL per entity type

#### 2. **Real-time Subscription Management**
**Current**: Multiple notifiers create overlapping Supabase Realtime subscriptions  
**Suggest**: Centralized subscription manager to prevent duplicate channels

#### 3. **Image Loading**
**Suggest**: Add proper image caching strategy for IGDB game covers
- Use cached_network_image consistently
...
#### 1. **Unused Imports**
`lib/main.dart` has unused import: `presentation/notifiers/lobby_notifier.dart`

#### 2. **Null Safety Patterns**
**Current**: Mix of `?` operators and safe helpers  
**Suggest**: Standardize on utils.dart safe helpers (safeDisplayName, etc.)

#### 3. **Error Handling**
**Current**: Try-catch with SnackBar messages  
**Suggest**: Implement centralized error handling service with user-friendly messages
...
**Critical**: Move hardcoded credentials to .env:
- IGDB API credentials in `igdb_service.dart`
- xAI Grok API key
- Firebase service account paths

#### 2. **RLS Policy Verification**
**Suggest**: Audit all Supabase RLS policies to ensure:
- Users can only access their own data
- Public data has proper read-only policies
...
**Current**: Basic test suite in test/ folder  
**Suggest**: Expand coverage for:
- Repository implementations
- Notifier business logic
- Service layer methods

#### 2. **Widget Testing**
**Suggest**: Add widget tests for:
- Complex UI components (chat_info_screen, lobby_sheet)
- Form validation (onboarding, profile)
...
### High Priority
- [ ] Replace all print() statements with logging framework
- [ ] Fix type compatibility issues (Lobby vs PublicSquad)
- [ ] Remove unused imports
- [ ] Move credentials to environment variables
- [ ] Update integration tests to repository pattern

### Medium Priority
- [ ] Split chat_info_screen.dart into modules
- [ ] Refactor large notifiers (ChatNotifier, SquadNotifier)
- [ ] Consolidate duplicate game selection components
- [ ] Centralize real-time subscription management
- [ ] Remove direct database access from notifiers

### Low Priority (Long-term)
- **12 analytics methods defined**
- **Only 2 methods used** (17%)
- **10 methods unused** (83%)
- **Impact**: Remove unused methods or implement usage

#### 🟡 **Service Consolidation Opportunities**
- `video_service.dart` (1,196 lines) + `voice_service.dart` (888 lines)
  - **Overlap**: Both implement Agora RTC, permissions, token generation
  - **Recommendation**: Create `AgoraRoomService` base class
  - **Impact**: -400 lines from consolidation

**Service Layer Total Savings**: ~1,400 lines

---

### 2. DOMAIN/DATA LAYER - OVER-ENGINEERED

#### 🔴 **CRITICAL: 89% of Use Cases Are Thin Wrappers**

**Analysis of 47 Use Case Files**:
```dart
// Typical example (add_pinned_game.dart - 8 lines):
class AddPinnedGame {
  final UserRepository _repository;
  AddPinnedGame(this._repository);
  Future<void> call(Map<String, dynamic> game) => 
      _repository.addPinnedGame(game);
}
```

**Breakdown**:
- **42 files** (89%): Pure delegation, zero business logic
- **5 files** (11%): Some validation/transformation logic
- **1 file** (`sync_games_to_firestore.dart`): Completely unused

**Recommendation**: 
- **DELETE 42 thin wrapper files** (~336 lines)
- Call repositories directly from notifiers
- Keep only 5 complex use cases
- **Impact**: -336 lines, simpler architecture

#### 🔴 **CRITICAL: Repository Interface Mismatches**

**LobbyRepository**:
```dart
// Interface defines (line 8):
Future<Lobby?> getSquadByInviteCode(String inviteCode);

// Implementation has (line 56):
Future<Lobby?> getLobbyByInviteCode(String inviteCode);

// Result: Use case calls non-existent method!
```

**ChatRepository**: 6 methods unimplemented (stubs with `throw UnimplementedError()`)

**Impact**: Runtime errors, broken features

#### 🟡 **Naming Inconsistency: Squad vs Lobby**
- Entity is called `Lobby`
- But methods alternate: `createLobby()`, `getSquad()`, `squadSpots`
- **Recommendation**: Standardize on "Lobby"

**Domain/Data Layer Total Savings**: ~400 lines + critical bug fixes

---

### 3. UI/PRESENTATION LAYER - MAJOR REFACTORING NEEDED

#### 🔴 **CRITICAL: God File**
`chat_info_screen.dart` (3,155 lines):
- **8+ internal widget classes**
- **Direct Supabase calls** throughout (violates architecture)
- **Should be**: 8 separate files (~1,000 total lines)
- **Impact**: -2,000 lines after proper componentization

#### 🔴 **CRITICAL: Duplicate Game Selection**
`add_game_screen.dart` (639 lines) + `game_selection_screen.dart` (866 lines):
- **Both implement**: IGDB search, popular games, grid layout
- **Redundant code**: ~500-600 lines
- **Recommendation**: Extract `GameSearchWidget` reusable component
- **Impact**: -600 lines

#### 🔴 **CRITICAL: Unused Onboarding**
`screens/onboarding/` (263 lines) - Simple version never used
- App uses fancy version in `presentation/onboarding/`
- **Impact**: -263 lines

#### 🔴 **CRITICAL: Duplicate Splash Screens**
- `widgets/splash_screen.dart` (92 lines) - Animated
- `screens/splash_screen.dart` (37 lines) - Static
- **Impact**: -50 lines after consolidation

#### 🟡 **Overly Large Screens**
- `video_room_screen.dart` (1,133 lines)
- `voice_room_screen.dart` (1,029 lines)
- `discovery_swipe_screen.dart` (762 lines)
- **Target**: Break down to <500 lines each
- **Impact**: Better maintainability

#### 🟡 **Theme Violations**
- **100+ hardcoded colors** across screens
- **49 BackdropFilter instances** with duplicate glass patterns
- **Recommendation**: Use theme system consistently
- **Impact**: Improved design consistency

#### 🔴 **Massive Notifiers**
- `chat_notifier.dart` (928 lines)
- `lobby_notifier.dart` (795 lines)
- **Target**: <400 lines each
- **Recommendation**: Extract managers for media, timers, streams
- **Impact**: Better code organization

**UI/Presentation Total Savings**: ~3,500 lines + better architecture

---

### 4. CHAT MODULE - ARCHITECTURAL VIOLATIONS

#### 🔴 **CRITICAL: Direct Database Access in UI Layer**
4 files bypass service/repository pattern:

1. `available_lobbies_widget.dart` (lines 79-118):
```dart
...
2. `invite_members_dialog.dart` (lines 212-232):
```dart
final response = await SupabaseService.client.from('users').select()
```

3. `chat_info_screen.dart` (lines 92-100):
```dart
final userData = await SupabaseService.client.from('users').select()
```

4. `peacock_modal.dart` (lines 100+):
```dart
await SupabaseService.client.from('peacocks').insert(peacockData);
```

**Impact**: Architectural violation, hard to test, security risk

#### 🔴 **CRITICAL: Duplicate Message Models**
- `chat/message.dart` (68 lines) - Simple model, 26 usages
- `chat/models/message_data.dart` (330 lines) - Complex model, 15 usages
- **Recommendation**: Keep `MessageData`, migrate 26 usages, delete simple version
- **Impact**: -68 lines, data consistency

#### 🟡 **Misplaced Lobby Files in Chat Module**
- `chat/lobbies_screen.dart` (631 lines)
- `chat/available_lobbies_widget.dart` (130 lines)
- `chat/lobby_sheet.dart` (318 lines)
- `chat/spots_sheet.dart` 
- **Recommendation**: Move to `lobbies_tab/` or delete if unused
- **Impact**: Better module organization

#### 🟡 **Unused Chat Services**
- `chat_state_notifier.dart` (210 lines) - Replaced by new architecture
- **Impact**: -210 lines

#### 🟡 **Example Files in Production**
- `clip_integration_example.dart` (217 lines)
- 5 markdown files in `chat/widgets/`
- **Recommendation**: Move to `docs/` directory
- **Impact**: Cleaner production code

**Chat Module Total Savings**: ~400 lines + architectural fixes

---

### 5. LOBBIES TAB MODULE - MOSTLY GOOD ✅

**Status**: Best-structured module in the codebase!

**Good Patterns Found**:
- ✅ Proper widget separation
- ✅ Dialog isolation in `dialogs/` subdirectory (16 dialogs)
- ✅ Manager pattern for navigation
- ✅ Mixin for keyboard handling
- ✅ No direct database access
- ✅ Uses notifiers correctly

**Minor Issue**:
- `lobbies_tab.dart` (824 lines) - Acceptable for main coordinator
- Could extract MembersSection if it grows larger

**Lobbies Tab Module**: No major issues! Use as reference for other modules.

---

### 6. CORE ARCHITECTURE - MINOR ISSUES

#### 🟢 **main.dart** (306 lines):
- ✅ Good initialization flow
- ✅ Proper error handling
- ✅ Firebase and Supabase dual setup
- No issues

#### 🟢 **utils.dart** (117 lines):
- ✅ Utility functions properly isolated
- ✅ Good null safety helpers
- Minor: Could extract badge logic to separate file

#### 🟡 **injection.dart** (476 lines):
- 47+ use case registrations (42 are unnecessary if use cases deleted)
- Otherwise well-structured
- **Impact**: Simplify after use case cleanup

**Core Architecture**: Solid foundation, minor cleanup needed
...
|----------|-------|-------|--------|
| Thin Wrapper Use Cases | 42 | ~336 | No business logic |
| Duplicate Services | 3 | ~769 | IgdbAuthService, AuthSyncService, ChatPersistence |
| Unused Services | 1 | ~462 | BackgroundService never used |
| God File Extraction | 7 | ~2,000 | Break up chat_info_screen |
| Duplicate Screens | 3 | ~400 | Game selection, onboarding, splash |
| Unused Chat Components | 2 | ~278 | chat_state_notifier, unused sheets |

**Total Files to Delete/Refactor**: 58+ files
**Total Lines to Remove**: 5,000-7,000 lines (12-16% of active code)

---

### Critical Bug Fixes Needed

1. **Repository method name mismatch** (LobbyRepository)
2. **6 unimplemented ChatRepository methods**
3. **Direct database access in 4 UI files**
4. **Hardcoded credentials in IgdbService** (security risk)
5. **Squad vs Lobby naming inconsistency** (100+ references)

---

### Estimated Developer Effort

| Priority | Category | Hours | Impact |
|----------|----------|-------|--------|
| 🔴 CRITICAL | Use case layer removal | 12-16h | -336 lines |
| 🔴 CRITICAL | Service duplication | 8-12h | -1,400 lines |
| 🔴 CRITICAL | God files refactor | 30-40h | -3,000 lines |
| 🟡 HIGH | UI component consolidation | 20-25h | -1,500 lines |
| 🟡 HIGH | Direct DB access fixes | 6-8h | Security |
| 🟢 MEDIUM | Unused code cleanup | 4-6h | -500 lines |

**Total Estimated Time**: 80-107 developer hours
**Total Impact**: 5,000-7,000 lines removed + critical fixes
...
1. **Delete thin wrapper use cases** (42 files)
   - Update notifiers to call repositories directly
   - Remove from dependency injection
   - **Impact**: -336 lines, simpler architecture

2. **Fix repository method mismatches**
   - `getSquadByInviteCode` → `getLobbyByInviteCode`
   - Implement or remove 6 ChatRepository stubs
   - **Impact**: Prevent runtime errors

3. **Remove direct database access**
   - Fix 4 UI files to use notifiers/repositories
   - **Impact**: Proper architecture, security

4. **Delete duplicate/unused services**
   - IgdbAuthService, AuthSyncService, ChatPersistenceService
   - **Impact**: -584 lines

### Phase 2: Major Refactoring (Week 2-3) - 35-45 hours

5. **Break up god file** (`chat_info_screen.dart`)
   - Extract 8 widgets to separate files
   - **Impact**: -2,000 lines, maintainable code

6. **Consolidate game selection**
   - Extract `GameSearchWidget` component
   - **Impact**: -600 lines

7. **Split large notifiers**
   - ChatNotifier (928 lines) → Extract managers
   - LobbyNotifier (795 lines) → Extract managers
   - **Impact**: Better organization

### Phase 3: UI Cleanup (Week 4) - 20-25 hours

8. **Consolidate UI duplicates**
   - Splash screens, onboarding flows
   - **Impact**: -313 lines

9. **Theme system enforcement**
   - Replace 100+ hardcoded colors
   - Extract glass components
   - **Impact**: Design consistency

10. **Remove unused components**
    - chat_state_notifier, example files
    - **Impact**: -400 lines

### Phase 4: Polish (Week 5) - 4-6 hours

11. **Move documentation files**
    - Examples, markdown to `docs/`
    - **Impact**: Cleaner codebase

12. **Naming standardization**
    - Squad → Lobby throughout
    - **Impact**: Consistency

---

## High Priority Redundancies (Historical - December 8-9, 2025)
