  ...
  Widget build(BuildContext context) {
    return Consumer(
      builder: (context, ref, child) {
        // Watch unified notifier and extract current lobby
        final lobbyStateAsync = ref.watch(lobbyNotifierProvider);

        if (lobbyStateAsync.hasError) return const Text('Error');
        if (lobbyStateAsync.isLoading) {
          return const Scaffold(
              body: Center(child: CircularProgressIndicator()));
        }

        final squad = lobbyStateAsync.value?.currentLobby;
        if (squad == null) {
          return const Scaffold(body: Center(child: Text('No lobby selected')));
        }

        return Scaffold(
          appBar: AppBar(
            title: Text(squad.name),
            leading: const BackButton(), // back to lobbies list
            actions: [
              IconButton(
              ...
              PopupMenuButton<String>(
                ...
                itemBuilder: (context) => [
                  ...
                  const PopupMenuItem(
                      value: 'leave_squad', child: Text('Leave Lobby')),
                ],
              ),
            ],
          ),
